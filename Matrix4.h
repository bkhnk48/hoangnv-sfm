...
template <class T> void Matrix4<T>::set(const Matrix4 &m1) {
  m00 = m1.m00;
  m01 = m1.m01;
  m02 = m1.m02;
  m03 = m1.m03;
...
template <class T> void Matrix4<T>::set(const T m[]) {
  assert(m != 0);
  m00 = m[0];
  m01 = m[1];
  m02 = m[2];
...
template <class T> void Matrix4<T>::set(const Matrix3<T> &m1) {
  m00 = m1.m00;
  m01 = m1.m01;
  m02 = m1.m02;
  m03 = 0.0;
...
template <class T> void Matrix4<T>::set(const Quat4<T> &q1) {
  setFromQuat(q1.x, q1.y, q1.z, q1.w);
}

template <class T> void Matrix4<T>::set(const AxisAngle4<T> &a1) {
...
template <class T> T Matrix4<T>::getElement(size_t row, size_t column) const {
  if (row == 0)
    if (column == 0)
      return m00;
    else if (column == 1)
...
template <class T> void Matrix4<T>::get(Matrix3<T> *m1) const {
  assert(m1 != 0);
  SVD(m1, 0);
}

template <class T> T Matrix4<T>::get(Matrix3<T> *m1, Vector3<T> *t1) const {
  assert(m1 != 0);
  assert(t1 != 0);
  get(t1);
  return SVD(m1, 0);
...
template <class T> void Matrix4<T>::get(Quat4<T> *q1) const {
  assert(q1 != 0);
...
template <class T> void Matrix4<T>::getRotationScale(Matrix3<T> *m1) const {
  assert(m1 != 0);
  m1->m00 = m00;
  m1->m01 = m01;
  m1->m02 = m02;
...
template <class T> T Matrix4<T>::getScale() const { return SVD(0, 0); }

template <class T> void Matrix4<T>::setRotationScale(const Matrix3<T> &m1) {
  m00 = m1.m00;
  m01 = m1.m01;
  m02 = m1.m02;
  m10 = m1.m10;
...
template <class T> void Matrix4<T>::setScale(T scale) {
  // note: this don't change m44.
  SVD(0, this);
  mulRotationScale(scale);
}

template <class T> void Matrix4<T>::setRow(size_t row, T x, T y, T z, T w) {
  assert(row < 4);
  if (row == 0) {
    m00 = x;
    m01 = y;
...
template <class T> void Matrix4<T>::setRow(size_t row, const Vector4<T> &v) {
  assert(row < 4);
  if (row == 0) {
    m00 = v.x;
    m01 = v.y;
...
template <class T> void Matrix4<T>::setRow(size_t row, const T v[]) {
  if (row == 0) {
    m00 = v[0];
    m01 = v[1];
    m02 = v[2];
...
template <class T> void Matrix4<T>::getRow(size_t row, Vector4<T> *v) const {
  assert(row < 4);
  assert(v != 0);
  if (row == 0) {
    v->x = m00;
...
template <class T> void Matrix4<T>::getRow(size_t row, T v[]) const {
  assert(row < 4);
  if (row == 0) {
    v[0] = m00;
    v[1] = m01;
...
template <class T> void Matrix4<T>::setColumn(size_t column, const T v[]) {
  if (column == 0) {
    m00 = v[0];
    m10 = v[1];
    m20 = v[2];
...
template <class T> void Matrix4<T>::getColumn(size_t column, T v[]) const {
  assert(column < 4);
  assert(v != 0);
  if (column == 0) {
    v[0] = m00;
...
template <class T> void Matrix4<T>::add(T scalar) {
  m00 += scalar;
  m01 += scalar;
  m02 += scalar;
  m03 += scalar;
...
template <class T> void Matrix4<T>::sub(T scalar) { add(-scalar); }

template <class T> void Matrix4<T>::add(const Matrix4 &m1, const Matrix4 &m2) {
...
template <class T> void Matrix4<T>::add(const Matrix4 &m1) {
  m00 += m1.m00;
  m01 += m1.m01;
  m02 += m1.m02;
  m03 += m1.m03;
...
template <class T> void Matrix4<T>::sub(const Matrix4 &m1, const Matrix4 &m2) {
  // note this is alias safe.
  set(m1.m00 - m2.m00, m1.m01 - m2.m01, m1.m02 - m2.m02, m1.m03 - m2.m03,
      m1.m10 - m2.m10, m1.m11 - m2.m11, m1.m12 - m2.m12, m1.m13 - m2.m13,
      m1.m20 - m2.m20, m1.m21 - m2.m21, m1.m22 - m2.m22, m1.m23 - m2.m23,
...
template <class T> void Matrix4<T>::sub(const Matrix4 &m1) {
  m00 -= m1.m00;
  m01 -= m1.m01;
  m02 -= m1.m02;
  m03 -= m1.m03;
...
template <class T> void Matrix4<T>::transpose() {
  T tmp = m01;
  m01 = m10;
  m10 = tmp;
  tmp = m02;
...
template <class T> void Matrix4<T>::invert(const Matrix4<T> &m1) {
  set(m1);
  invert();
}

template <class T> void Matrix4<T>::invert() {
  T s = determinant();
  if (s == 0.0)
    return;
  s = 1 / s;
...
template <class T> void Matrix4<T>::set(T scale) {
  m00 = scale;
  m01 = 0.0;
  m02 = 0.0;
  m03 = 0.0;
...
template <class T> void Matrix4<T>::set(const Vector3<T> &v1, T scale) {
  m00 = scale;
  m01 = 0.0;
  m02 = 0.0;
  m03 = scale * v1.x;
...
template <class T> void Matrix4<T>::rotX(T angle) {
...
template <class T> void Matrix4<T>::rotY(T angle) {
  T c = VmUtil<T>::cos(angle);
  T s = VmUtil<T>::sin(angle);
  m00 = c;
  m01 = 0.0;
...
template <class T> void Matrix4<T>::rotZ(T angle) {
  T c = VmUtil<T>::cos(angle);
  T s = VmUtil<T>::sin(angle);
  m00 = c;
  m01 = -s;
...
template <class T> void Matrix4<T>::mul(T scalar) {
  m00 *= scalar;
  m01 *= scalar;
  m02 *= scalar;
  m03 *= scalar;
...
template <class T> void Matrix4<T>::mul(const Matrix4 &m1, const Matrix4 &m2) {
  // alias-safe way
...
template <class T> bool Matrix4<T>::equals(const Matrix4<T> &m1) const {
...
template <class T> void Matrix4<T>::setRotation(const Matrix3<T> &m1) {
  T scale = SVD(0, 0);
  setRotationScale(m1);
  mulRotationScale(scale);
}

template <class T> void Matrix4<T>::setRotation(const Quat4<T> &q1) {
  T scale = SVD(0, 0);
  // save other values
  T tx = m03;
  T ty = m13;
...
template <class T> void Matrix4<T>::setRotation(const AxisAngle4<T> &a1) {
  T scale = SVD(0, 0);
  // save other values
  T tx = m03;
  T ty = m13;
...
template <class T> void Matrix4<T>::setZero() {
  m00 = 0.0;
  m01 = 0.0;
  m02 = 0.0;
  m03 = 0.0;
...
template <class T> void Matrix4<T>::negate() {
  m00 = -m00;
  m01 = -m01;
  m02 = -m02;
  m03 = -m03;
...
template <class T> T Matrix4<T>::SVD(Matrix3<T> *rot3, Matrix4 *rot4) const {
...
template <class T> void Matrix4<T>::mulRotationScale(T scale) {
  m00 *= scale;
  m01 *= scale;
  m02 *= scale;
  m10 *= scale;
...
template <class T> void Matrix4<T>::setRotationScale(const Matrix4 &m1) {
  m00 = m1.m00;
  m01 = m1.m01;
  m02 = m1.m02;
  m10 = m1.m10;
...
template <class T> void Matrix4<T>::setFromQuat(T x, T y, T z, T w) {
  T n = x * x + y * y + z * z + w * w;
  T s = (n > 0.0) ? (2.0 / n) : 0.0;
...
template <class T> void Matrix4<T>::setFromAxisAngle(T x, T y, T z, T angle) {
