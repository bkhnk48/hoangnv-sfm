...
void Agent::setRadius(float radius) { this->radius = radius; }

void Agent::setDesiredSpeed(float speed) { desiredSpeed = speed; }

void Agent::setColour(float red, float green, float blue) {
  colour.set(red, green, blue);
}

void Agent::setPosition(float x, float y) { position.set(x, y, 0.0); }
...
Point3f Agent::getPath() {
  ...
  if (distanceCurr.lengthSquared() <
      (path.front().radius * path.front().radius)) {
    path.push_back(path.front());
    path.pop_front();
  }
...
Point3f Agent::getAheadVector() const { return (velocity + position); }

void Agent::move(vector<Agent *> agents, vector<Wall *> walls,
                 vector<AGV *> agvs, float stepTime) {
  Vector3f acceleration;

  // Compute Social Force
  acceleration = drivingForce(getPath()) + agentInteractForce(agents) +
                 wallInteractForce(walls) + agvInteractForce(agvs);
...
Vector3f Agent::drivingForce(const Point3f position_target) {
  ...
  // Formula: e_i = (position_target - position_i) / ||(position_target -
  // position_i)||
...
Vector3f Agent::agentInteractForce(vector<Agent *> agents) {
  ...
  for (const Agent *agent_j : agents) {
    ...
    if (agent_j->id != id) {
      ...
      // Formula: f_theta = -A * K * exp(-distance_ij / B - ((n * B * theta) *
      // (n * B * theta)))
      f_theta =
          -A * K *
...
Vector3f Agent::wallInteractForce(vector<Wall *> walls) {
  // const float repulsionRange = 0.3F;	// Repulsion range based on (Moussaid et
  // al., 2009)
  const int a = 3;
  const float b = 0.1F;
...
Vector3f Agent::agvInteractForce(vector<AGV *> agvs) {
  // Constant Values Based on (Moussaid et al., 2009)
  const float lambda = 2.0; // Weight reflecting relative importance of velocity
                            // vector against position vector
  const float gamma = 0.35F; // Speed interaction
