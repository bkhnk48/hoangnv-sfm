...
template <class T> void Matrix3<T>::set(const Matrix3 &m1) {
  m00 = m1.m00;
  m01 = m1.m01;
  m02 = m1.m02;
  m10 = m1.m10;
...
template <class T> void Matrix3<T>::set(const T v[]) {
  assert(v != 0);
  m00 = v[0];
  m01 = v[1];
  m02 = v[2];
...
template <class T> void Matrix3<T>::setScale(T scale) {
  SVD(this);
  m00 *= scale;
  m11 *= scale;
  m22 *= scale;
...
template <class T> void Matrix3<T>::set(const Quat4<T> &q1) {
  setFromQuat(q1.x, q1.y, q1.z, q1.w);
}

template <class T> void Matrix3<T>::set(const AxisAngle4<T> &a1) {
...
template <class T> T Matrix3<T>::getElement(size_t row, size_t column) const {
  assert(row < 3);
  assert(column < 3);
  if (row == 0)
    if (column == 0)
...
template <class T> void Matrix3<T>::setRow(size_t row, T x, T y, T z) {
  assert(row < 3);
  if (row == 0) {
    m00 = x;
    m01 = y;
...
template <class T> void Matrix3<T>::setRow(size_t row, const Vector3<T> &v) {
  assert(row < 3);
  if (row == 0) {
    m00 = v.x;
    m01 = v.y;
...
template <class T> void Matrix3<T>::setRow(size_t row, const T v[]) {
  assert(v != 0);
  assert(row < 3);
  if (row == 0) {
    m00 = v[0];
...
template <class T> void Matrix3<T>::getRow(size_t row, T v[]) const {
  assert(v != 0);
  assert(row < 3);
  if (row == 0) {
    v[0] = m00;
...
template <class T> void Matrix3<T>::getRow(size_t row, Vector3<T> *v) const {
  assert(v != 0);
  assert(row < 3);
  if (row == 0) {
    v->x = m00;
...
template <class T> void Matrix3<T>::setColumn(size_t column, T x, T y, T z) {
  assert(column < 3);
  if (column == 0) {
    m00 = x;
    m10 = y;
...
template <class T> void Matrix3<T>::setColumn(size_t column, const T v[]) {
  assert(v != 0);
  assert(column < 3);
  if (column == 0) {
    m00 = v[0];
...
template <class T> void Matrix3<T>::getColumn(size_t column, T v[]) const {
  assert(column < 3);
  assert(v != 0);
  if (column == 0) {
    v[0] = m00;
...
template <class T> T Matrix3<T>::getScale() const { return SVD(0); }

template <class T> void Matrix3<T>::add(T scalar) {
  m00 += scalar;
  m01 += scalar;
  m02 += scalar;
  m10 += scalar;
...
template <class T> void Matrix3<T>::sub(T scalar) { add(-scalar); }

template <class T> void Matrix3<T>::add(T scalar, const Matrix3 &m1) {
  set(m1);
  add(scalar);
}

template <class T> void Matrix3<T>::add(const Matrix3 &m1, const Matrix3 &m2) {
...
template <class T> void Matrix3<T>::add(const Matrix3 &m1) {
  m00 += m1.m00;
  m01 += m1.m01;
  m02 += m1.m02;
  m10 += m1.m10;
...
template <class T> void Matrix3<T>::sub(const Matrix3 &m1, const Matrix3 &m2) {
  // note this is alias safe.
  set(m1.m00 - m2.m00, m1.m01 - m2.m01, m1.m02 - m2.m02, m1.m10 - m2.m10,
      m1.m11 - m2.m11, m1.m12 - m2.m12, m1.m20 - m2.m20, m1.m21 - m2.m21,
      m1.m22 - m2.m22);
...
template <class T> void Matrix3<T>::sub(const Matrix3 &m1) {
  m00 -= m1.m00;
  m01 -= m1.m01;
  m02 -= m1.m02;
  m10 -= m1.m10;
...
template <class T> void Matrix3<T>::transpose() {
  T tmp = m01;
  m01 = m10;
  m10 = tmp;
...
template <class T> void Matrix3<T>::transpose(const Matrix3 &m1) {
  // alias-safe
  set(m1);
  transpose();
}

template <class T> void Matrix3<T>::invert(const Matrix3 &m1) {
  // alias-safe way.
  set(m1);
  invert();
}

template <class T> void Matrix3<T>::invert() {
  T s = determinant();
  // zero div may happen
  s = 1 / s;
  // alias-safe way.
...
template <class T> void Matrix3<T>::set(T scale) {
  m00 = scale;
  m01 = 0.0;
  m02 = 0.0;
  m10 = 0.0;
...
template <class T> void Matrix3<T>::rotX(T angle) {
...
template <class T> void Matrix3<T>::rotY(T angle) {
  T c = VmUtil<T>::cos(angle);
  T s = VmUtil<T>::sin(angle);
  m00 = c;
  m01 = 0.0;
...
template <class T> void Matrix3<T>::rotZ(T angle) {
  T c = VmUtil<T>::cos(angle);
  T s = VmUtil<T>::sin(angle);
  m00 = c;
  m01 = -s;
...
template <class T> void Matrix3<T>::mul(T scalar) {
  m00 *= scalar;
  m01 *= scalar;
  m02 *= scalar;
  m10 *= scalar;
...
template <class T> void Matrix3<T>::mul(T scalar, const Matrix3 &m1) {
  set(m1);
  mul(scalar);
}

template <class T> void Matrix3<T>::mul(const Matrix3 &m1) { mul(*this, m1); }

template <class T> void Matrix3<T>::mul(const Matrix3 &m1, const Matrix3 &m2) {
  // alias-safe way.
...
template <class T> bool Matrix3<T>::equals(const Matrix3 &m1) const {
...
template <class T> void Matrix3<T>::setZero() {
  m00 = 0.0;
  m01 = 0.0;
  m02 = 0.0;
  m10 = 0.0;
...
template <class T> void Matrix3<T>::negate() {
  m00 = -m00;
  m01 = -m01;
  m02 = -m02;
  m10 = -m10;
...
template <class T> T Matrix3<T>::SVD(Matrix3 *rot) const {
...
template <class T> void Matrix3<T>::setFromQuat(T x, T y, T z, T w) {
  T n = x * x + y * y + z * z + w * w;
  T s = (n > 0.0) ? (2.0 / n) : 0.0;
...
template <class T> void Matrix3<T>::setFromAxisAngle(T x, T y, T z, T angle) {
