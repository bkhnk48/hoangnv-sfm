...
template <class T> void Quat4<T>::mul(const Quat4 &q1, const Quat4 &q2) {
...
template <class T> void Quat4<T>::mul(const Quat4 &q1) {
  // store on stack for aliasing-safty
...
template <class T> void Quat4<T>::mulInverse(const Quat4 &q1, const Quat4 &q2) {
  T n = norm();
  // zero-div may occur.
  n = 1 / n;
...
template <class T> void Quat4<T>::mulInverse(const Quat4 &q1) {
  T n = norm();
  // zero-div may occur.
  n = 1 / n;
  // store on stack once for aliasing-safty
...
template <class T> void Quat4<T>::set(const Matrix4<T> &m1) {
  setFromMat(m1.m00, m1.m01, m1.m02, m1.m10, m1.m11, m1.m12, m1.m20, m1.m21,
             m1.m22);
}

template <class T> void Quat4<T>::set(const Matrix3<T> &m1) {
  setFromMat(m1.m00, m1.m01, m1.m02, m1.m10, m1.m11, m1.m12, m1.m20, m1.m21,
             m1.m22);
}

template <class T> void Quat4<T>::set(const AxisAngle4<T> &a1) {
  this->x = a1.x;
  this->y = a1.y;
  this->z = a1.z;
...
template <class T> void Quat4<T>::interpolate(const Quat4 &q1, T alpha) {
  // From Hoggar.
  normalize();
...
template <class T> Quat4<T> &Quat4<T>::operator*=(const Quat4 &m1) {
  mul(m1);
  return *this;
}

template <class T> Quat4<T> Quat4<T>::operator*(const Quat4 &m1) const {
